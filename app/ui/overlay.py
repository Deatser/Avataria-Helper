...
# пишут «В разработке» и не открываются ни по клику, ни из автозагрузки —
# см. _toggle_module.
...
class Overlay(GameFitMixin, CollapseMixin, CrtPowerMixin, BackgroundDragMixin,
    ...
    def __init__(self, config, window_manager, stats=None, parent=None):
        ...
        # Окно ежедневного подарка караулится всегда, а не только пока
        # включён какой-то мод: оно встаёт поверх игры само по себе, и
        # увидеть его некому, если каждый мод смотрит только за своим экраном.
    ...
    def _on_reward_stuck(self, score: float):
        ...
        self._reward_log(f"Окно подарка не закрылось ({score:.0%}) — "
                         f"забрать подарок не вышло", level="error",
    ...
    def closeEvent(self, event):
        ...
        if not self.config.data.overlay.skip_close_confirm and \
