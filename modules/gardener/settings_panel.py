...
class GardenerSettingsPanel(SheetPanel):
    """Садовник's settings sheet: what shows on the window, and what the
    bot is allowed to start on its own."""
    ...
    def _build_body(self, layout: QVBoxLayout):
        ...
        self._kind_switch = NtSwitch(_KIND_BUTTONS_TEXT, accent=theme.GD_OLIVE)
        ...
        self._auto_switch = NtSwitch(_AUTO_CLEAN_TEXT, accent=theme.GD_OLIVE)
        ...
        layout.addWidget(self.hint(
            "Бот будет следить, когда сад снова станет доступен для уборки, "
            "и в этот момент сам переключится на Садовника и начнёт "
            "уборку — где бы аватар ни находился."))
