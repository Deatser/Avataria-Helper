...
def _default_backdrop(video: bool = True) -> str:
    """First existing templates/snowboard_sinthwawe.* file.

    With video on, moving formats come first; with it off, stills do — the
    same rule Ava Dancers' own backdrop picker uses, see its module for why:
    either way something existing beats an empty panel.
    """
...
class SnowboardWindow(ModuleWindow):
    ...
    def _build_ui(self):
        ...
        # asynchronously and the earlier it is told to start, the more
        # likely a real frame is already waiting the moment the
        # switch-on animation asks backdrop_ready whether to hold for one.
